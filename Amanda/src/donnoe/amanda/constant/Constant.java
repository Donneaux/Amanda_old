...
public abstract class Constant extends Blob {
    ...
    private static final Map<Integer, BiFunction<ClassFile, Integer, Constant>> CONSTANT_CONSTRUCTORS =
        unmodifiableMap(new HashMap<Integer, BiFunction<ClassFile, Integer, Constant>>(
            new HashMap<Integer, Function<ClassFile, Constant>>(of(0x09, 0x0A, 0x0B).collect(toMap(
                Function.identity(),
                k -> ReferenceConstant::new
            ))) {{
                ...
                Entry::getKey,
                e -> (cF, index) -> e.getValue().apply(cF)
            ))
